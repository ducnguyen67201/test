...
check_prerequisites() {
    ...
    # Check env file
    if [[ ! -f "$ENV_FILE" ]]; then
        log_error ".env.prod not found. Copy from template:"
        log_error "  cp .env.prod.example .env.prod"
        log_error "  # Edit .env.prod with your secrets"
        exit 1
    fi
...
main() {
    ...
    case "${1:-}" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Prerequisites:"
            echo "  1. Copy .env.prod.example to .env.prod and configure secrets"

...
async def teardown_worker_tick() -> int:
    ...
    for lab_ref in lab_refs:
        lab_id = lab_ref.id
        # Reconstruct a minimal Lab-like object for runtime calls
        class _LabMinimal:
            def __init__(self, id):
                self.id = id
        lab_min = _LabMinimal(lab_id)

        try:
            # Reconcile: if resources are already gone, finalize without calling destroy
            try:
                resources_exist = await runtime.resources_exist_for_lab(lab_min) if hasattr(runtime, 'resources_exist_for_lab') else True
            except Exception:
            ...
            # Build a lightweight Lab for destroy call using minimal attributes expected by runtime
            class _LabForDestroy:
                def __init__(self, id):
                    self.id = id
            lab_for_destroy = _LabForDestroy(lab_id)

            teardown_result = await asyncio.wait_for(
                runtime.destroy_lab(lab_for_destroy),
                timeout=settings.teardown_timeout_seconds,
            )

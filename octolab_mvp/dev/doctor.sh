...
check_env_files() {
    ...
    # Check consolidated .env exists
    if [[ -f "$ENV_FILE" ]]; then
        check_pass "backend/.env exists"

        # Check permissions (should be 600 for files with secrets)
        local perms
        perms=$(stat -c "%a" "$ENV_FILE" 2>/dev/null || stat -f "%OLp" "$ENV_FILE" 2>/dev/null || echo "unknown")
        if [[ "$perms" == "600" ]]; then
            check_pass "backend/.env permissions 600"
        else
            check_warn "backend/.env permissions $perms (should be 600 for production)"
        fi
    else
        check_fail "backend/.env missing (copy from backend/.env.example)"
    fi

    # Check GUAC_ENC_KEY
    if [[ -f "$ENV_FILE" ]]; then
        local guac_enc_key
        guac_enc_key=$(grep -E "^GUAC_ENC_KEY=" "$ENV_FILE" 2>/dev/null | cut -d= -f2- | tr -d '"' | tr -d "'" || echo "")
...
" 2>/dev/null || echo "error")
            ...
            check_warn "GUAC_ENC_KEY not set in .env"
        fi
    fi
}
...
check_database() {
    ...
    # Try .env first
    if [[ -f "$ENV_FILE" ]]; then
        db_url=$(grep -E "^DATABASE_URL=" "$ENV_FILE" 2>/dev/null | cut -d= -f2- | tr -d '"' | tr -d "'" || echo "")
    fi
...
check_guacamole() {
    ...
    if [[ -f "$ENV_FILE" ]]; then
        guac_enabled=$(grep -E "^GUAC_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d= -f2- | tr -d '"' | tr -d "'" || echo "")
    fi

...
LabStatusStr = Literal[
    ...
    "provisioning",
    "ready",
    "degraded",  # OctoBox works but target crashed
    "ending",
    "finished",
    "failed",
]
...
class SourceFile(BaseModel):
    """A source file to include in Docker build context."""

    filename: str = Field(..., max_length=255, description="Filename (no paths)")
    content: str = Field(..., max_length=1_000_000, description="File content")


class LabCreateFromDockerfile(BaseModel):
    """Schema for creating a lab from LLM-generated Dockerfile.

    Used by octo-web frontend to deploy labs with custom Dockerfiles.
    """

    dockerfile: str = Field(
        ...,
        max_length=65536,
        description="Dockerfile content (max 64KB)",
    )
    source_files: list[SourceFile] = Field(
        default_factory=list,
        max_length=20,
        description="Additional source files referenced by Dockerfile",
    )
    recipe_name: str = Field(
        ...,
        max_length=256,
        description="Name for the recipe/lab",
    )
    software: str = Field(
        ...,
        max_length=128,
        description="Software being targeted (e.g., 'Apache httpd')",
    )
    version_constraint: str | None = Field(
        default=None,
        max_length=64,
        description="Vulnerable version constraint (e.g., '2.4.49')",
    )
    exploit_family: str | None = Field(
        default=None,
        max_length=128,
        description="Exploit family (e.g., 'Path Traversal')",
    )
    base_image: str | None = Field(
        default=None,
        max_length=256,
        description="Base Docker image used",
    )

...
class LabOrchestrator:
    ...
    async def stop_lab(self, lab: Lab, session: AsyncSession) -> Lab:
        """Stop the HackVM stack for a lab."""
        logger.info("Stopping lab %s via docker compose", lab.id)
        try:
            await self._run_compose(("docker", "compose", "down"))
        except subprocess.CalledProcessError as exc:
            logger.warning(
                "docker compose down failed for lab %s (continuing): %s",
                lab.id,
                exc.stderr or exc.stdout,
            )
